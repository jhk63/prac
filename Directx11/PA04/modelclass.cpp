...
XMFLOAT3 ModelClass::GetPosition() const 
{
	return m_position;
}
...
void ModelClass::UpdateRotation(float x, float y, float z) 
{
	m_rotation.x += x;
	m_rotation.y += y;
	m_rotation.z += z;
...
bool ModelClass::InitializeBuffers(ID3D11Device* device)
{
	...
	{
		instances[i].position = XMFLOAT3((i * 2.0f) - 10.0f, 0.0f, 5.0f);
		//instances[i].rotation = XMFLOAT3(0.0f, XM_PI * (i / 10.0f), 0.0f);
	}
...
bool ModelClass::InitializeGround(ID3D11Device* device, float width, float depth, const WCHAR* textureFilename) 
{
	VertexType* vertices;
	unsigned long* indices;
	D3D11_BUFFER_DESC vertexBufferDesc, indexBufferDesc;
	...
	vertices[0].position = XMFLOAT3(-halfWidth, yPos, -halfDepth); // Bottom-left
	vertices[0].texture = XMFLOAT2(0.0f, 1.0f);
	vertices[0].normal = XMFLOAT3(0.0f, 1.0f, 0.0f);

	vertices[1].position = XMFLOAT3(halfWidth, yPos, -halfDepth); // Bottom-right
	vertices[1].texture = XMFLOAT2(1.0f, 1.0f);
	vertices[1].normal = XMFLOAT3(0.0f, 1.0f, 0.0f);

	vertices[2].position = XMFLOAT3(halfWidth, yPos, halfDepth); // Top-right
	vertices[2].texture = XMFLOAT2(1.0f, 0.0f);
	vertices[2].normal = XMFLOAT3(0.0f, 1.0f, 0.0f);

	vertices[3].position = XMFLOAT3(-halfWidth, yPos, -halfDepth); // Bottom-left
	vertices[3].texture = XMFLOAT2(0.0f, 1.0f);
	vertices[3].normal = XMFLOAT3(0.0f, 1.0f, 0.0f);

	vertices[4].position = XMFLOAT3(halfWidth, yPos, halfDepth); // Top-right
	vertices[4].texture = XMFLOAT2(1.0f, 0.0f);
	vertices[4].normal = XMFLOAT3(0.0f, 1.0f, 0.0f);

	vertices[5].position = XMFLOAT3(-halfWidth, yPos, halfDepth); // Top-left
	...
	for (int i = 0; i < m_instanceCount; i++)
	{
		instances[i].position = XMFLOAT3((i * 10.0f), 0.0f, 0.0f);
		//instances[i].rotation = XMFLOAT3(0.0f, XM_PI * (i / 10.0f), 0.0f);
	}
